...
export const check = function (value, pattern) {
  ...
  try {
    ...
    if ((err instanceof Match.Error) && err.path) { err.message += ' in field ' + err.path }
    throw err
  }
}
...
var checkSubtree = function (value, pattern) {
  ...
  if (pattern instanceof Optional) { pattern = Match.OneOf(undefined, pattern.pattern) }
  ...
  if (typeof pattern !== 'object') { throw Error('Bad pattern: unknown pattern type') }

  // An object, with required and optional keys. Note that this does NOT do
  // structural matches against objects of special types that happen to match
  // the pattern: this really needs to be a plain old {Object}!
  if (typeof value !== 'object') { throw new Match.Error('Expected object, got ' + typeof value) }
  if (value === null) { throw new Match.Error('Expected object, got null') }
  if (value.constructor !== Object) { throw new Match.Error('Expected plain object') }

  var requiredPatterns = {}
  var optionalPatterns = {}
  pattern.forEach(function (subPattern, key) {
    if (subPattern instanceof Optional) { optionalPatterns[key] = subPattern.pattern } else { requiredPatterns[key] = subPattern }
  })
  ...
  value.forEach(function (subValue, key) {
    try {
      if (_.has(requiredPatterns, key)) {
        ...
        if (!unknownKeysAllowed) { throw new Match.Error('Unknown key') }
  ...
  requiredPatterns.forEach(function (subPattern, key) {
...
Object.assign(ArgumentChecker.prototype, {
  checking: function (value) {
    ...
    // itself an argument.
    if (Array.isArray(value) || Object.prototype.toString.call(value) === '[object Arguments]') {
      value.forEach(() => self._checkingOneValue.bind(self))
    }
  },
  _checkingOneValue: function (value) {
    ...
    for (var i = 0; i < self.args.length; ++i) {
      ...
      if (value === self.args[i] || (isNaN(value) && isNaN(self.args[i]))) {
  ...
  throwUnlessAllArgumentsHaveBeenChecked: function () {
    var self = this
    if (!Object.keys(self.args).length) {
